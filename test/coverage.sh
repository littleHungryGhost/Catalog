...
gs_ctl start -D "$DATADIR" -l "$LOG_FILE" -o "-p 5432" 2>&1 | tail -1
sleep 1
echo "  数据库已启动"
...
echo ""
echo "=== [5/8] 创建测试数据库 ==="
gsql -d postgres -p 5432 -c "DROP DATABASE IF EXISTS $TEST_DB;" > /dev/null 2>&1 || true
gsql -d postgres -p 5432 -c "CREATE DATABASE $TEST_DB;" > /dev/null 2>&1
	gsql -d "$TEST_DB" -p 5432 -c "CREATE EXTENSION iceberg_fdw;" 2>&1
gsql -d "$TEST_DB" -p 5432 -c "CREATE EXTENSION iceberg_catalog;" 2>&1
echo "  扩展已安装"
...
for f in "$PROJECT_ROOT"/test/sql/*.sql; do
    name=$(basename "$f" .sql)
    OUTPUT=$(gsql -d "$TEST_DB" -p 5432 -f "$f" 2>&1) || true
